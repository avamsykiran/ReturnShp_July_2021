...
public interface SubscriptionRepo extends JpaRepository<Subscription, SubscriptionId> {
	...
	@Query("SELECT new com.cts.srad.model.SubscriptionDetailsForSubscriber(s.subpId.channel.channelName,s.subscriptionActivationDate) FROM Subscription s WHERE s.subpId.subscriber.subscriberId=:subId")
